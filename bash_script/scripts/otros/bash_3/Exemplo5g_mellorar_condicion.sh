...
[[ $n1 =~ ^-?[0-9]+$ ]] &&              #Co dobre corchete e mediante expresión regular verifícase se a variable n1 é un número enteiro:
                                        ...
                                        ## && -> Equivale a then
   [ $(uname -m) = 'x86_64' -a $n1 -ge -9223372036854775808 -a $n1 -le 9223372036854775807 ] 2>/dev/null ||  ## If anidado con simple corchete e and (-a). Soamente é verdadeiro se se cumpren todas as condicións. Ademais mediante || tamén débese validar outra condición/s sendo || equivalente a or. Así ou ven as condicións da liña 20 se cumpren ou débense cumprir as condicións da liña 26. A maiores se non se cumpren as condicións enviamos a saída estándar das mensaxes de erros (stderr) ao dispositivo especial /dev/null para descartalos.
   ...
   [ $n1 -ge -2147483648 -a  $n1 -le 2147483647 ] 2>/dev/null && 
    ...
    ##2>/dev/null -> Descartar a saída estándar das mensaxes de erros (stderr) ao dispositivo especial /dev/null para descartalos.
    ...
    ) || (echo $? && echo 'Solicitouse un número')     # )   -> Finaliza a execución do código "then"
                                          # ||  -> Equivale a else
                                          ##echo $? && echo -> Demostramos a existencia do erro amosando o valor de $? e se o introducido non é un número enteiro amósase a mensaxe: Solicitouse un número
